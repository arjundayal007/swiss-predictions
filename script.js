  ...
  if (currentRound <= INITIAL_ROUNDS) {
    if (currentRound === 1) {
      ...
      const winners = round1Matches
        .filter((m) => m.outcome === "Team1Win" || m.outcome === "Bye")
        .map((m) => m.team1);
      const losers = round1Matches
        .filter((m) => m.outcome === "Team2Win")
        .map((m) => m.team2);
...
function submitMatchResult(matchId) {
  ...
  if (match.team2) {
    const score1 = parseInt(
      document.getElementById(`score1-${match.id}`).value
    );
    const score2 = parseInt(
      document.getElementById(`score2-${match.id}`).value
    );
